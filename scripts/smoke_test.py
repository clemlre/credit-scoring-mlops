"""Test de fumée d'une instance de l'API en service (conteneur CI ou Space déployé).

Bibliothèque standard uniquement. Le dossier envoyé est construit à partir de
GET /features.

    python scripts/smoke_test.py [URL]    # défaut : http://127.0.0.1:8000
"""
...
def wait_until_ready(base_url: str) -> None:
    ...
    for attempt in range(1, STARTUP_ATTEMPTS + 1):
        try:
            ...
            raison = exc.__class__.__name__
            print(f"  tentative {attempt}/{STARTUP_ATTEMPTS} : pas encore joignable ({raison})")
...
def main(base_url: str) -> int:
    ...
    historique = contrat["history_features"]
    print(f"  {len(application)} features de dossier, {len(historique)} d'historique")
