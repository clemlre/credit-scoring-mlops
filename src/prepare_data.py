...
def _flatten(agg: pd.DataFrame, prefix: str) -> pd.Index:
    return pd.Index([f"{prefix}{col}_{stat.upper()}" for col, stat in agg.columns])
...
def bureau_and_balance(num_rows: int | None = None, nan_as_category: bool = True) -> pd.DataFrame:
    ...
    bb_agg.columns = _flatten(bb_agg, "")
    ...
    bureau_agg.columns = _flatten(bureau_agg, "BURO_")

    active = bureau[bureau["CREDIT_ACTIVE_Active"] == 1]
    active_agg = active.groupby("SK_ID_CURR").agg(num_aggregations)
    active_agg.columns = _flatten(active_agg, "ACTIVE_")
    ...
    closed_agg.columns = _flatten(closed_agg, "CLOSED_")
    bureau_agg = bureau_agg.join(closed_agg, how="left", on="SK_ID_CURR")
    return bureau_agg


def previous_applications(num_rows: int | None = None) -> pd.DataFrame:
    ...
    prev_agg.columns = _flatten(prev_agg, "PREV_")

    approved = prev[prev["NAME_CONTRACT_STATUS_Approved"] == 1]
    approved_agg = approved.groupby("SK_ID_CURR").agg(num_aggregations)
    approved_agg.columns = _flatten(approved_agg, "APPROVED_")
    prev_agg = prev_agg.join(approved_agg, how="left", on="SK_ID_CURR")

    refused = prev[prev["NAME_CONTRACT_STATUS_Refused"] == 1]
    refused_agg = refused.groupby("SK_ID_CURR").agg(num_aggregations)
    refused_agg.columns = _flatten(refused_agg, "REFUSED_")
    prev_agg = prev_agg.join(refused_agg, how="left", on="SK_ID_CURR")
    return prev_agg


def pos_cash(num_rows: int | None = None) -> pd.DataFrame:
    ...
    pos_agg.columns = _flatten(pos_agg, "POS_")
    pos_agg["POS_COUNT"] = pos.groupby("SK_ID_CURR").size()
    return pos_agg


def installments_payments(num_rows: int | None = None) -> pd.DataFrame:
    ...
    ins_agg.columns = _flatten(ins_agg, "INSTAL_")
    ins_agg["INSTAL_COUNT"] = ins.groupby("SK_ID_CURR").size()
    return ins_agg


def credit_card_balance(num_rows: int | None = None) -> pd.DataFrame:
    cc = pd.read_csv(DATA_DIR / "credit_card_balance.csv", nrows=num_rows)
    cc, _ = one_hot_encoder(cc)
    cc.drop(["SK_ID_PREV"], axis=1, inplace=True)
    cc_agg = cc.groupby("SK_ID_CURR").agg(["min", "max", "mean", "sum", "var"])
    cc_agg.columns = _flatten(cc_agg, "CC_")
